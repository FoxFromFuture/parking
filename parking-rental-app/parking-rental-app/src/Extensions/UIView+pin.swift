...
//  parking-rental-app
//
//  Created by Никита Лисунов on 2/13/24.
//
...
extension UIView {
    ...
    func pinHeight(
        ...
        return pinConstraint(mode: mode, heightAnchor, dimension, multiplier: mult)
    }
    ...
    private func pinConstraint<Axis: AnyObject, AnyAnchor: NSLayoutAnchor<Axis>>(
        mode: ConstraintMode,
        _ firstDimension: AnyAnchor,
        _ secondDimension: AnyAnchor,
        constant: Double = 0
    ) -> NSLayoutConstraint {
        ...
        switch mode {
        case .equal:
            result = firstDimension.constraint(equalTo: secondDimension, constant: const)
        case .grOE:
            result = firstDimension.constraint(greaterThanOrEqualTo: secondDimension, constant: const)
        case .lsOE:
            result = firstDimension.constraint(lessThanOrEqualTo: secondDimension, constant: const)
        }
